    ...
    private renderSingleCanvas(
        ...
        if (canvas.width > 0 && canvas.height > 0) {
            ...
            if (progressCtx) {
                ...
                const imageData = progressCtx.getImageData(0, 0, Math.min(10, canvas.width), Math.min(10, canvas.height))
                const hasContent = imageData.data.some((val, i) => i % 4 === 3 && val > 0)
                
                if (!hasContent) {
                    console.warn('Progress canvas is empty, canvas might not be rendered yet')
                }
                
                progressCtx.globalCompositeOperation = 'source-over'
                progressCtx.fillStyle = this.convertColorValues(options.progressColor)
                progressCtx.fillRect(0, 0, canvas.width, canvas.height)
                progressCtx.globalCompositeOperation = 'destination-in'
                progressCtx.drawImage(canvas, 0, 0)
            }
